...
class Service {
    ...
    Parameters = (req, res) => {
        ...
        if (urlParts.query.x !== undefined && urlParts.query.y !== undefined) {
            x = urlParts.query.x;
            y = urlParts.query.y;
            source = 'query'; //из строки запроса
        } else {
            let params = urlParts.pathname.split('/');
            if (params.length >= 3) {
                x = params[2];
                y = params[3];
                source = 'route'; //из пути
            }
        }
    ...
    XML = (req, res) => {
        ...
        req.on('end', () => {
            ...
            //<?xml version="1.0" encoding="UTF-8"?><request id="28"><x value="1"/><x value="2"/><m value="a"/><m value="b"/><m value="c"/></request>
        })
    }
