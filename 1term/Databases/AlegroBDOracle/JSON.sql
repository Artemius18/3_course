...
BEGIN
  v_file := UTL_FILE.FOPEN('UTL_DIR', 'JSON_ALLEGRO.json', 'W', 4096);
  ...
  LOOP
    ...
    v_json := v_json || '"GENRE_ID":"' || NVL(REPLACE(v_row.GENRE_ID, '"', '\"'), 'NULL') || '",';
    v_json := v_json || '"SONG":"' || NVL(REPLACE(v_row.SONG, '"', '\"'), 'NULL') || '",';
    v_json := v_json || '"SONG_COVER":"' || NVL(REPLACE(v_row.SONG_COVER, '"', '\"'), 'NULL') || '",';
    v_json := v_json || '"SONG_NAME":"' || NVL(REPLACE(v_row.SONG_NAME, '"', '\"'), 'NULL') || '",';
    v_json := v_json || '"LISTENING_COUNTER":"' || NVL(REPLACE(v_row.LISTENING_COUNTER, '"', '\"'), 'NULL') || '",';
    v_json := v_json || '"AUTHOR_ID":"' || NVL(REPLACE(v_row.AUTHOR_ID, '"', '\"'), 'NULL') || '"';
